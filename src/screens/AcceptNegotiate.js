...
import { negotiateByStudent, acceptOffer } from "../Redux/Actions/Tutors";
...
const AcceptNegotiate = ({ route }) => {
    ...
    console.log(All_Booked_Tutor[0], 'Allbooked')
    ...
    useEffect(() => {
        if (All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable') {
            setValueR('')
        }
    }, [])
            ...
            <View style={[styles.Bookcard, styles.BookshadowProp]}>
                <View
                    ...
                    <Text style={styles.BookText1}>Message:
                        <Text style={styles.BookText2}>Tutor has made a new offer, The Offer is {All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type}</Text>
                ...
                <ScrollView style={{ marginBottom: 20 }}>
                    <View style={{ display: 'flex', flexDirection: 'row' }}>
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', height: 50, backgroundColor: "#2F5597", marginTop: wp(5), justifyContent: "center" }}>
                                ...
                                {
                                   valueR === ''  ?
                                        <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600', alignSelf: "center", marginLeft: 5 }}>{All_Booked_Tutor[0]?.amount_negotiate_by_student === '0.00'? All_Booked_Tutor[0]?.tutor_tution_offer_amount : All_Booked_Tutor[0]?.amount_negotiate_by_student}</Text>
                                        :
                                        <TextInput
                                            style={{
                        ...
                        <View style={{
                            ...
                            <Image
                                ...
                                style={styles.Dollaricons}
                            />
                            <Text style={{ fontSize: 22, textAlign: "center", fontWeight: '700', color: 'red' }}>Tutor's Offer</Text>

                            <View style={{ flexDirection: 'row', height: 50, backgroundColor: "red", marginTop: wp(5), justifyContent: "center" }}>

                                <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600', alignSelf: "center", }}>SGD</Text>
                                <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600', alignSelf: "center", marginLeft: 5 }}>{All_Booked_Tutor[0]?.amount_negotiate_by_tutor}</Text>


                                <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600', alignSelf: "center", }}>/ hour</Text>

                            </View>
                            <Text style={{ textAlign: "center", color: "red", fontSize: 14, marginTop: wp(3) }}>{All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type}</Text>
                    ...
                    {
                        All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable' ?
                            <View></View>
                            :
                            valueR === 'second' ?
                                <View></View>
                                :
                                <RadioButton.Group onValueChange={value => setValueR(value)} value={valueR} >
                                    <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginTop: 20 }}>
                                        <View style={styles.radioBtn}>
                                            <Text style={{ color: '#2F5597', marginTop: 7 }}>Accept</Text>
                                            <RadioButton value="first" />
                                        </View>
                                        <View style={styles.radioBtn}>
                                            <Text style={{ color: '#2F5597', marginTop: 7 }}>Negotiate</Text>
...

                    {
                        ...
                        All_Booked_Tutor[0]?.offer_status === 'Accept' ?
                            <View style={{ marginTop: wp(15), height: wp(12), backgroundColor: '#2F5597', borderColor: '#2F5597', borderWidth: 1, flexDirection: 'row', marginBottom: 7, width: wp(100), alignSelf: 'center', justifyContent: "center" }}>
                                <View style={{ width: wp(60), height: wp(8), alignSelf: 'center', justifyContent: "center" }}>
                                    <Text style={{ color: '#fff', fontSize: 13, fontWeight: '600' }}> Agreed Fees is SGD {All_Booked_Tutor[0]?.amount_negotiate_by_student}/ hour </Text>
            ...
            >
              <Text style={{ color: "#fff", fontSize: 15, fontWeight: "500" }}>
                ...
                <View
                    ...
                    <TouchableOpacity
                        // disabled={All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable' ? true : false}
                        onPress={() => {
                            if (All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable') {
                                AcceptOffer()
                            } else {
                        ...
                        style={{
                            ...
                            justifyContent: 'center',
                            alignItems: 'center',
                            backgroundColor: All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable' ? '#F6BE00' : '#fff',
                            borderRadius: 3,
                        }}>
                        <Text style={styles.infoText1}>{All_Booked_Tutor[0]?.negotiate_by_tutor_amount_type === 'Non Negotiable' ? 'Accept' : 'Next'}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </SafeAreaView>
...
export default AcceptNegotiate;
