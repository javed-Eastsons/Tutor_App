...
import AcceptNon from './AcceptNon';

var selectArray = [];
var selectFilter = [];

const AcceptNonTutor = ({route}) => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const [userdata, setUserdata] = useState([]);
  const [postaldata, setPostaldata] = useState([]);
  const [offerAmount, setofferAmount] = useState(route.params.amount);
  const [youroffer, setYourOffer] = useState(route.params.youramount);
...
/>
    ...
    <Text style={{textAlign:"center",color:"darkgreen",fontSize:14,marginTop:wp(3)}}>{value}</Text>    
     ...
     <View style={{marginTop:wp(5),backgroundColor:'#2F5597',height:wp(12),borderColor:'#2F5597',borderWidth:1,flexDirection:'row',}}> 
     <Text style={{width:wp(90),color:'#fff',fontSize:16,fontWeight:'500',margin:10,textAlign:"center"}}>Agreed Fee is SGD {youroffer}.00  /hour</Text>
        ...
        <View
          ...
          <TouchableOpacity
            onPress={() =>Alert.alert('Move to Date/Time')
